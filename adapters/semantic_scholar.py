...
def _s2_headers(api_key: Optional[str] = None) -> dict:
    hdrs = dict(_HEADERS)
    if api_key:
...
def _post_with_retry(
    ...
    return _fetch_with_retry(
        url, headers=_s2_headers(api_key), params=params, json_body=json_body,
        method="POST", max_retries=max_retries, timeout=timeout,
        return_none_on_404=True, rate_limit_codes=_RETRY_CODES,
    )
...
def _get_with_retry(
    ...
    return _fetch_with_retry(
        url, headers=_s2_headers(api_key), params=params,
...
def fetch_abstracts_batch(
    ...
    for batch_start in range(0, len(doi_papers), BATCH_SIZE):
        ...
        for i, entry in enumerate(data):
            ...
            # DOI URLs are landing-page redirects, not actual PDF files
