...
def process_year(year: str) -> list[dict]:
    ...
    # fetch papers concurrently for much faster procesing
    logger.info(f"  Fetching {len(entries)} papers concurrently...")
    entry_by_hash = {e["hash"]: e for e in entries}
    results = fetch_parallel(
        list(entry_by_hash.keys()),
        lambda h: fetch_and_parse_paper(entry_by_hash[h], year, use_metadata_json),
        max_workers=20, default=None, progress_interval=100,
    )
